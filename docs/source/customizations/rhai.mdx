...
sequenceDiagram
    ...
    Client->>RS: Sends request
    RS->>SupergraphService: Transforms raw HTTP<br/>request into GraphQL/JSON<br/>request
    ...
    SupergraphService-->>RS: Returns GraphQL/JSON<br/>response
    RS-->>Client: Returns HTTP<br/>response
```
...
In addition to the examples below, see more examples in the Router repo's [examples directory](https://github.com/duan-jason/router/tree/main/examples). Rhai-specific examples are listed in `README.md`.
...
This example converts cookies into headers for transmission to subgraphs. There is a complete working example (with tests) of this in the [examples/cookies-to-headers directory](https://github.com/duan-jason/router/tree/main/examples/cookies-to-headers).
