...
There is a complete [helm chart definition](https://github.com/duan-jason/router/tree/v1.15.1/helm/chart/router) in the repo which illustrates how to use helm to deploy the router in kubernetes.
...
```bash
helm install --set router.configuration.telemetry.metrics.prometheus.enabled=true --set managedFederation.apiKey="REDACTED" --set managedFederation.graphRef="REDACTED" --create-namespace --namespace router-deploy router-test oci://ghcr.io/duan-jason/helm-charts/router --version 1.0.0-rc.8 --values router/values.yaml
```
    ...
    spec:
      ...
      containers:
        - name: router
          securityContext:
            {}
          image: "ghcr.io/duan-jason/router:v1.15.1"
